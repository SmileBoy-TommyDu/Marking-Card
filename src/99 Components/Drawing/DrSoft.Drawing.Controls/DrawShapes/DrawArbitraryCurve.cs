...
namespace DrSoft.Drawing.Controls.DrawShapes
{
    /// <summary>
    /// 任意曲线图形（自由绘制）。
    /// 存储用户鼠标轨迹的密集采样点，使用 Catmull-Rom 样条平滑渲染。
    /// 与 DrawBezier（锚点式贝塞尔）不同，任意曲线适合手绘/自由绘制场景。
    /// 支持缩放/旋转/倾斜变换，局部坐标系存储确保变换后几何精确。
    /// </summary>
    public class DrawArbitraryCurve : DrawObject, IHatchable, IClosable, IArbitraryCurveShapeData
    {

        /// <summary>是否闭合</summary>
        public bool IsClosed { get; set; } = false;

        // ── IArbitraryCurveShapeData：代理到 IsClosed ────
        bool IArbitraryCurveShapeData.IsClosed => IsClosed;

        // 局部坐标点（相对于 SharpCenter）
        private List<SKPoint> _localPoints;

        // 基准尺寸，用于控制点拖动时的比例缩放
        private float _baseWidth;
        private float _baseHeight;

        public DrawArbitraryCurve()
        {
            UId = UniqueIdGenerator.NextId();
            Type = ShapeType.ArbitraryCurve;
        }

        public DrawArbitraryCurve(List<SKPoint> points) : this()
        {
            Points = points ?? new List<SKPoint>();
        ...
        {
            var clone = new DrawArbitraryCurve()
            {
                IsClosed = IsClosed,
        ...
        public override void UpdateSetProperty(List<SKPoint> points)
        {
            ...
            else
            {
                ...
                foreach (var point in Points)
                {
                    _localPoints.Add(new SKPoint(point.X, point.Y));
                }
            }

            // 将世界坐标的点转换为局部坐标（相对于 SharpCenter）
        ...
        /// 用于 GetPath(SKPaintCache) 路径池化复用。
        /// </summary>
        protected override void FillPath(SKPath path)
        {
            ...
            // 不在 FillPath 内部做 Width/Height 缩放——
        ...
        /// <summary>
        /// 获取所有锚点的真实世界坐标（包含旋转/缩放/倾斜变换）
        /// 不同于 Points（Points 仅包含平移变换，不包含旋转）
        /// </summary>
        ...
        {
            return new DrawArbitraryCurveMemento(this);
        }

        protected class DrawArbitraryCurveMemento : DrawObjectMemento
        {
            ...
            public DrawArbitraryCurveMemento(DrawArbitraryCurve curve) : base(curve)
            {
                _isClosed = curve.IsClosed;
                if (curve._localPoints != null)
                {
                    _localPoints = new List<SKPoint>(curve._localPoints.Count);
                    foreach (var p in curve._localPoints)
                        _localPoints.Add(new SKPoint(p.X, p.Y));
                }
                _baseWidth = curve._baseWidth;
                _baseHeight = curve._baseHeight;
            }

            protected override void RestoreDerived()
            {
                if (Shape is DrawArbitraryCurve curve)
                {
                    curve.IsClosed = _isClosed;
                    // 恢复正确的 _localPoints，确保旋转/缩放后的图形渲染正确
                    if (_localPoints != null)
                    {
                        curve._localPoints = new List<SKPoint>(_localPoints.Count);
                        foreach (var p in _localPoints)
                            curve._localPoints.Add(new SKPoint(p.X, p.Y));
                        curve._baseWidth = _baseWidth;
                        curve._baseHeight = _baseHeight;
                    }
                }
            }
        }
