...
namespace DrSoft.Docking.Model.Layout
{
    public class LayoutGroup : BaseLayoutGroup
    {
        ...
        protected override void OnChildrenCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            if (_view is BaseGroupControl groupControl)
            {
                groupControl.RefreshMinimumSizeFromContent();
            }
        ...
        public override void Detach(IDockElement element)
        {
            ...
            if (_view != null)
            {
                ...
                var isApplyingLayout = _dockManager != null && _dockManager.IsApplyingLayout;
                bool heightReachedMin = bgc.ActualHeight >= Constants.MinDockHeight;
                bool widthReachedMin = bgc.ActualWidth >= Constants.MinDockWidth;

                if (!isApplyingLayout && bgc.IsInitialized && heightReachedMin && widthReachedMin)
                {
