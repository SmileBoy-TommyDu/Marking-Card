...
namespace DrSoft.Docking.View
{
    public class BaseGroupControl : TabControl, ILayoutGroupControl, IDragTarget
    {
        internal BaseGroupControl(ILayoutGroup model, double desiredWidth = Constants.DockDefaultWidthLength, double desiredHeight = Constants.DockDefaultHeightLength)
        {
            ...
            var resolvedDesiredWidth = desiredWidth;
            var resolvedDesiredHeight = desiredHeight;

            if (model.Children.Count() > 0)
            {
                var firstChild = model.Children.First();
                resolvedDesiredWidth = firstChild.DesiredWidth;
                resolvedDesiredHeight = firstChild.DesiredHeight;
            }

            _desiredWidth = resolvedDesiredWidth;
            _desiredHeight = resolvedDesiredHeight;
        ...
        {
            RefreshMinimumSizeFromContent();
        }

        internal void RefreshMinimumSizeFromContent()
        {
            var resolvedModel = _model;
            if (resolvedModel == null)
            {
                MinWidth = Constants.MinDockWidth;
                MinHeight = Constants.MinDockHeight;
                return;
            }

            double minWidth = Constants.MinDockWidth;
            double minHeight = Constants.MinDockHeight;

            foreach (var child in resolvedModel.Children)
            {
                if (child is not DockElement dockElement)
                {
                    continue;
                }

                if (dockElement.Content is not FrameworkElement contentElement)
                {
                    continue;
                }

                minWidth = Math.Max(minWidth, contentElement.MinWidth);
                minHeight = Math.Max(minHeight, contentElement.MinHeight);

                if (contentElement is not ContentControl contentControl)
                {
                    continue;
                }

                if (contentControl.Content is not FrameworkElement innerContentElement)
                {
                    continue;
                }

                minWidth = Math.Max(minWidth, innerContentElement.MinWidth);
                minHeight = Math.Max(minHeight, innerContentElement.MinHeight);
            }

            var dockSources = resolvedModel.Children
                .OfType<DockElement>()
                .Select(item => item.Content as IDockSource)
                .Where(item => item != null);

            foreach (var dockSource in dockSources)
            {
                if (dockSource.OuterMinWidth > 0)
                {
                    minWidth = Math.Max(minWidth, dockSource.OuterMinWidth);
                }

                if (dockSource.OuterMinHeight > 0)
                {
                    minHeight = Math.Max(minHeight, dockSource.OuterMinHeight);
                }
            }
        ...
        public double DesiredWidth
        {
            get
            {
                var effectiveWidth = Math.Max(_desiredWidth, MinWidth);
                return effectiveWidth;
            }
            set
            {
                var normalizedWidth = Math.Max(0, value);
                if (_desiredWidth != normalizedWidth)
                {
                    _desiredWidth = normalizedWidth;
                    var effectiveWidth = Math.Max(_desiredWidth, MinWidth);
                    if (_model != null)
                        foreach (ILayoutSize item in _model.Children)
                            item.DesiredWidth = effectiveWidth;
                }
            }
        }
        ...
        public double DesiredHeight
        {
            get
            {
                var effectiveHeight = Math.Max(_desiredHeight, MinHeight);
                return effectiveHeight;
            }
            set
            {
                var normalizedHeight = Math.Max(0, value);
                if (_desiredHeight != normalizedHeight)
                {
                    _desiredHeight = normalizedHeight;
                    var effectiveHeight = Math.Max(_desiredHeight, MinHeight);
                    if (_model != null)
                        foreach (ILayoutSize item in _model.Children)
                            item.DesiredHeight = effectiveHeight;
                }
            }
        }
        ...
        protected double ResolveAttachSeedWidth()
        {
            ...
            return Constants.DockDefaultWidthLength;
        }

        protected double ResolveAttachSeedHeight()
        {
            ...
            return Constants.DockDefaultHeightLength;
        }
